...
// FIX: Removed unused icons 'CheckCircle', 'XCircle', and 'Bot' from this import.
...
const HuntPlanetsGame = () => {
  ...
  useEffect(() => {
    const loadTop3 = async () => {
      ...
      if (!error && data) {
        setLeaderboard(
          data.map((d: any) => ({ name: d.username, score: d.score }))
        );
      }
    };
    loadTop3();
  ...
  useEffect(() => {
    const fetchCsv = async () => {
      try {
        ...
        if (!res) {
          setCsvError(
            "CSV not found. Place planetsQuestions.csv in public/data/ (or public/)."
          );
          return;
        }
  ...
  ); // const derivedCandidate = useMemo(() => { //   if (header.length === 0 || currentRow.length === 0) return MOCK_GAME_CANDIDATE; //   const disposition = (field('disposition') || '').toUpperCase(); //   const isPlanet = disposition === 'CONFIRMED'; //   const pl_orbper = field('pl_orbper'); //   const pl_rade = field('pl_rade'); //   const st_teff = field('st_teff'); //   const st_rad = field('st_rad'); //   const pl_eqt = field('pl_eqt'); //   const imgFromCsv = field('image'); //   const name = field('pl_name') || field('name') || 'Unknown Candidate'; //   // Compute image URL: use explicit 'image' column if present, otherwise try name-based file in public/ //   const baseUrl = (import.meta as any).env?.BASE_URL || '/'; //   const toSlug = (s: string) => s.trim().replace(/\s+/g, '_'); //   const guessImage = `${toSlug(name)}.jpg`; //   const imageUrl = `${baseUrl}${imgFromCsv && imgFromCsv.trim().length ? imgFromCsv.trim() : guessImage}`; //   const features: { feature: string; importance: number }[] = []; //   if (pl_orbper) features.push({ feature: `Orbital Period: ${pl_orbper} d`, importance: 0.30 }); //   if (pl_rade) features.push({ feature: `Planet Radius (R⊕): ${pl_rade}`, importance: 0.25 }); //   if (st_teff) features.push({ feature: `Stellar Teff (K): ${st_teff}`, importance: 0.20 }); //   if (st_rad) features.push({ feature: `Stellar Radius (R☉): ${st_rad}`, importance: 0.15 }); //   if (pl_eqt) features.push({ feature: `Equilibrium Temp (K): ${pl_eqt}`, importance: 0.10 }); //   const normalized = features.map((f, i) => ({ ...f, importance: features[i].importance })); //   return { //     id: name || 'cand_csv', //     lightCurveUrl: imageUrl, //     aiPrediction: { //       isPlanet, //       confidence: 0.9, //       reasoning: normalized, //     }, //   } as typeof MOCK_GAME_CANDIDATE; // // eslint-disable-next-line react-hooks/exhaustive-deps // }, [header, currentRowIndex, csvRows]); // Build a map of available images in /src/data/Images using Vite's glob. // This ensures we use actual served URLs rather than assuming paths.

  // FIX: This entire block is commented out because the 'derivedCandidate' variable was never used.
  ...
  const makeFeatureKey = (p?: PlanetData | null) => {
    ...
    const orb = Math.round(
      (parseFloat(p.orbital_period_days || "0") || 0) * 100
    );
  ...
  const chooseImageByFeatures = (planet?: PlanetData | null): string | null => {
    ...
    const norm = (n: number) =>
      isNaN(n) || !isFinite(n) ? 0 : Math.abs(Math.floor(n));
  ...
  const getPlanetImageCandidates = (planet?: PlanetData | null): string[] => {
    const defaultImage =
      imageNameToUrl["k2-168b.png"] ||
      Object.values(imageNameToUrl)[0] ||
      "/favicon.ico";
    const raw = (
    ...
    const chosenByFeatures =
      mapped ||
      (typeof planet === "object" ? chooseImageByFeatures(planet) : null);
  ...
  const imageCandidates = currentPlanet
    ? getPlanetImageCandidates(currentPlanet)
    : [];
